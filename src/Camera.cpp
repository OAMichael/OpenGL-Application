...
void Camera::setPosition(const glm::vec3& pos)  { Position_ = pos;   };

glm::vec3 Camera::getDirection() const   { return Direction_; };
void Camera::setDirection(const glm::vec3& dir) { Direction_ = dir;  };

glm::vec3 Camera::getRight() const       { return Right_; };
void Camera::setRight(const glm::vec3& r) { Right_ = r; };

glm::vec3 Camera::getUp() const { return Up_; };
void Camera::setUp(const glm::vec3& u) { Up_ = u; };
...
void Camera::setSensitivity(const float& s) { Sensitivity_ = s; };

float Camera::getAspect() const { return Aspect_; };
void Camera::setAspect(const float& a) { Aspect_ = a; };

float Camera::getZNear() const { return ZNear_; };
void Camera::setZNear(const float& n) { ZNear_ = n; };

float Camera::getZFar() const { return ZFar_; };
void Camera::setZFar(const float& f) { ZFar_ = f; };

float Camera::getFov() const { return Fov_; };
void Camera::setFov(const float& f) { Fov_ = f; };

void Camera::setDirSpeed(const float& new_speed) { DirSpeedFactor_ = new_speed; };
void Camera::setRightSpeed(const float& new_speed) { RightSpeedFactor_ = new_speed; };

float Camera::getYaw() const { return Yaw_; };
void Camera::setYaw(const float& yaw) { Yaw_ = yaw; };

float Camera::getPitch() const { return Pitch_; };
void Camera::setPitch(const float& pitch) { Pitch_ = pitch; };
